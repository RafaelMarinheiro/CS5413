...
Proxy_Bridge_t Proxy_Create_Bridge(Proxy_Acceptor_t acceptor,
	...
	Proxy_Bridge_t ret = malloc(sizeof(struct _proxy_bridge_t));
	...
	// sem_init(&(ret->signal[0]), 0, 0);
	// sem_init(&(ret->signal[1]), 0, 0);
...
int Proxy_Destroy_Bridge(Proxy_Bridge_t bridge){
	// sem_destroy(&(bridge->signal[0]));
	// sem_destroy(&(bridge->signal[1]));
